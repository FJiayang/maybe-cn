class Period
  ...
  def label
    if key_metadata
      I18n.t("activemodel.labels.period.#{key}.label", default: key_metadata.fetch(:label))
    else
      I18n.t("activemodel.labels.period.custom.label", default: "Custom Period")
    end
  end

  def label_short
    if key_metadata
      I18n.t("activemodel.labels.period.#{key}.label_short", default: key_metadata.fetch(:label_short))
    else
      I18n.t("activemodel.labels.period.custom.label_short", default: "Custom")
    end
  end

  def comparison_label
    if key_metadata
      I18n.t("activemodel.labels.period.#{key}.comparison_label", default: key_metadata.fetch(:comparison_label))
    else
  ...
  private
    ...
    def must_be_valid_date_range
      ...
      unless start_date.is_a?(Date) && end_date.is_a?(Date)
        errors.add(:start_date, :must_be_valid_date, value: start_date.inspect)
        errors.add(:end_date, :must_be_valid_date, value: end_date.inspect)
        return
      end
